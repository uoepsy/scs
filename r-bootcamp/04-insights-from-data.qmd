...
ggplot(.) +
    geom_histogram(aes(x = tstat, y = after_stat(density)), color = 'white') +
    labs(x = 'Sample mean') +
    geom_function(fun = ~ dt(., df = nrow(btdata) - 1), color = 'red', 
                  size = 1.5) +
    annotate(geom = 'text', x = 2.5, y = 0.1, label = 't(49) distribution',
             color = 'red', size = 5) + 
    theme_void()
```
...
::: {.callout-tip title="CIs and hypothesis tests" collapse="true"}
...
- [4.1 Exploratory Data Analysis](https://posit.cloud/learn/primers/3.1)
- [4.2 Bar Charts](https://posit.cloud/learn/primers/3.2)
- [4.3 Histograms](https://posit.cloud/learn/primers/3.3)
- [4.4 Boxplots and counts](https://posit.cloud/learn/primers/3.4)
